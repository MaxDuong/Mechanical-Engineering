...
public class Server : MonoBehaviour {
    ...
    // Data can be obtained from the clients such as Python or Matlab
    private void OnIncomingData(ServerClient c, byte [] ReceivedData) {
        ...
        switch(cleanData[0]) {
            case "ModRobot":
                ModifyRobot(float.Parse(cleanData[1]), float.Parse(cleanData[2]), float.Parse(cleanData[3]), float.Parse(cleanData[4]), float.Parse(cleanData[5]), float.Parse(cleanData[6]));
                Debug.Log("ModRobot");
                break;
            case "ModGrab":
                ModifyGrab(int.Parse(cleanData[1]));
                Debug.Log("ModGrab" + cleanData[1]);
                break;
            case "Camera":
                Debug.Log("Camera");

                // TakeScreenshotAndLogObjectPosition();
                StartCoroutine(TakeScreenshotAndLogObjectPosition(c));
                break;
        }
    }
    ...
    private IEnumerator TakeScreenshotAndLogObjectPosition(ServerClient c) {
        // string fileName = $"Screenshot";
        // string filePath = $"{Application.dataPath}/Images/{fileName}.png";

        // ScreenCapture.CaptureScreenshot(filePath);
        // Debug.Log("Screenshot saved to: " + filePath);
        
        // // Wait for a frame to ensure screenshot is saved
        // yield return new WaitForEndOfFrame();
        
        // // Convert filePath to bytes and send it back to Python
        // string response = $"{fileName}|{filePath}";
        // byte[] responseBytes  = Encoding.UTF8.GetBytes(response);
        // OutgoingData(c, responseBytes );
        // Debug.Log("Sent filePath to Python already");

        string fileName = "Screenshot";
        string filePath = Path.Combine(Application.dataPath, "Images", $"{fileName}.png");
        ...
        try {
            ...
            if (File.Exists(filePath)) {
                ...
                Debug.LogError("Screenshot file was not created at: " + filePath);
            }
        }
        catch (Exception e) {
